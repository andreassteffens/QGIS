...
void sbAddressServicesGui::doOsmInfo(const QgsPointXY& point)
{
	...
	strUrl += "reverse?q=";
	strUrl += "&accept-language=" + currentLocale + ",en";
	strUrl += "&format=json&zoom=18&addressdetails=1&extratags=1";
...
bool sbAddressServicesGui::processOsmInfoReply(const QString& strReply)
{
	...
	if (!jsonObject["extratags"].isNull())
	{
		...
		QJsonObject jsonExtras = jsonObject["extratags"].toObject();
		for (int i = 0; i < jsonExtras.keys().count(); i++)
		{
			if (!strExtras.isEmpty())
				strExtras += "\n";

			QString strKey = jsonExtras.keys()[i];
			QString strValue = jsonExtras[strKey].toString();

			if (!(strKey.isEmpty() || strKey.isNull()) && !(strValue.isEmpty() || strValue.isNull()))
